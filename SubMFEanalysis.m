...
for it = 1:1:2
    ...
    for is = 1:1:2
        ...
        mE2Ett(it,:)  = squeeze(mE2Ett(it,:)) + reshape(mE2E(it,is,:),1,5000);
        mE2I(it,is,:) = mEbin_ra(:,is)*DIE(it,is) + mIY(it)*fI(it);
        mE2Itt(it,:)  = squeeze(mE2Itt(it,:)) + reshape(mE2I(it,is,:),1,5000);
        
        mI2E(it,is,:) = mIbin_ra(:,is)*DEI(it,is);
        mI2Ett(it,:)  = squeeze(mI2Ett(it,:)) + reshape(mI2E(it,is,:),1,5000);
        mI2I(it,is,:) = mIbin_ra(:,is)*DII(it,is);
        mI2Itt(it,:)  = squeeze(mI2Itt(it,:)) + reshape(mI2I(it,is,:),1,5000);
    end
end
...
ylim([0 1])
% plot(mE2Itt(1,1+(ibin-1)*50000:50000*ibin),'R');
% hold on;
% plot(mI2Itt(1,1+(ibin-1)*50000:50000*ibin),'.-M');
...
plot(VEavgbin_ra(1+(ibin-1)*5000:5000*ibin,2),'b');
ylim([0 1])
% plot(mE2Ett(2,1+(ibin-1)*50000:50000*ibin),'b');
% hold on;
% plot(mI2Ett(2,1+(ibin-1)*50000:50000*ibin),'.-C');
...
ylim([0 40])
subplot(3,1,3);
plot(VIavgbin_ra(1+(ibin-1)*5000:5000*ibin,2),'R');
ylim([0 1])
